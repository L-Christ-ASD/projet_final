...
output "instance_info" {
  value = {
    ...
    PreProd_ip  = aws_instance.terrafom_preprod[*].public_ip
  }
}
...
resource "null_resource" "generate_ansible_inventory" {
  depends_on = [aws_instance.terrafom, aws_instance.terrafom_preprod]

  provisioner "local-exec" {
    command = <<EOT
      mkdir -p ./ansible
      echo "[instances]" > ../ansible/inventory.ini
      ${join("\n", formatlist("echo %s ansible_user=ubuntu ansible_ssh_private_key_file=${path.module}/vockey.pem >> ../ansible/inventory.ini", aws_instance.terrafom[*].public_ip))}
