...
resource "null_resource" "generate_ansible_inventory" {
  ...
  provisioner "local-exec" {
    command = <<EOT
      ...
      ${join("\n", formatlist("echo %s ansible_user=ubuntu ansible_ssh_private_key_file=../terrafom_preprod/vockey.pem ansible_ssh_extra_args='\"-o StrictHostKeyChecking=no\"' >> ../ansible/inventory", aws_instance.terrafom_preprod[*].public_ip))}
    EOT
  }
}
